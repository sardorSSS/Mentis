...
from app.database.models.assesment import DtmExam, SectionExam,BlockExam,ModulExam,TopicTest,CurrentRating
...
def add_dtm_exam_db(student_id: int, subject_id: int, common_subject_score: float,
    second_subject_score: float, first_subject_score: float,
                    total_score: float ,exam_date: Optional[datetime] = None,
                    category_correct: Optional[List[int]] = None, category_mistake: Optional[List[int]] = None) -> DtmExam:
    with next(get_db()) as db:
        new_exam = DtmExam(student_id=student_id,subject_id=subject_id,common_score =common_subject_score,
                           second_subject_score=second_subject_score,first_subject_score=first_subject_score,
                           exam_date=exam_date, total_score= total_score, category_correct = category_correct,
...
def edit_dtm_exam_db(exam_id: int,student_id: int,subject_id: int, common_score: Optional[float] = None,
    second_subject_score: Optional[float] = None, total_score :Optional[float] = None,
                     first_subject_score: Optional[float] = None, exam_date: Optional[datetime] = None,
                     category_correct: Optional[List[int]] = None, category_mistake: Optional[List[int]] = None) -> DtmExam:
    with next(get_db()) as db:
        exam = (db.query(DtmExam).filter_by(exam_id=exam_id,student_id=student_id,
                  subject_id=subject_id).first())
...
def get_common_score_by_student_and_date(student_id: int, exam_date: datetime) -> float:
    ...
    with next(get_db()) as db:
        ...
        return exam.common_subject_score
...
def add_section_exam_db(student_id: int, section_id: int, score: float, exam_date: datetime,
                        category_correct: Optional[List[int]] = None,
                        category_mistake: Optional[List[int]] = None) -> SectionExam:
    """
    ...
    with next(get_db()) as db:
        ...
        exam = SectionExam(student_id=student_id, section_id=section_id, score=score,
                           exam_date=exam_date,category_correct = category_correct,
                           category_mistake = category_mistake)
        db.add(exam)
        db.commit()
        db.refresh(exam)
...
def update_section_exam_db(section_exam_id: int,score: float | None = None,
                           section_id: int | None = None,student_id: int | None = None,
                           exam_date: datetime | None = None) -> SectionExam:
    """
...
def get_section_exam_score_db(student_id: int, exam_date: datetime) -> float:
    ...
    """
    with next(get_db()) as db:
        exam = (db.query(SectionExam).filter(SectionExam.student_id == student_id,SectionExam.exam_date == exam_date).first())
        if not exam:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
...
def get_avg_score_by_student_subject_db(student_id: int, subject_id: int) -> float:
    ...
    """
    with next(get_db()) as db:
        avg_score = (db.query(func.avg(SectionExam.score))
            .join(SectionExam.section).filter(SectionExam.student_id == student_id,Section.subject_id == subject_id).scalar())
        if avg_score is None:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
...
def add_block_exam_db(student_id: int,block_id: int,subject_id: int,
                      score: float,exam_date: datetime , category_correct: List[Any],category_mistake: List[Any]) -> BlockExam:
    """
...
def update_block_exam_db(block_exam_id: int,student_id: Optional[int] = None,block_id: Optional[int] = None,
        subject_id: Optional[int] = None,score: Optional[float] = None,exam_date: Optional[datetime] = None) -> BlockExam:
    """
    ...
    with next(get_db()) as db:
        exam = (db.query(BlockExam)
            .filter_by(student_id=student_id,
                subject_id=subject_id,exam_date=exam_date).first())
        if not exam:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                                detail=(f"No BlockExam for "f"student_id={student_id}, "
...
def get_avg_block_score_by_student_subject_db(student_id: int,subject_id: int) -> float:
    ...
    """
    with next(get_db()) as db:
        avg_score = (db.query(func.avg(BlockExam.score))
            .filter(BlockExam.student_id == student_id,
                BlockExam.subject_id == subject_id).scalar())
        if avg_score is None:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                                detail=(f"No BlockExams for "f"student_id={student_id}, "
...
def get_avg_block_score_for_subject_db(subject_id: int) -> Dict[int, float]:
    ...
    """
    with next(get_db()) as db:
        rows = (db.query(BlockExam.student_id,func.avg(BlockExam.score).label("avg_score")).filter(BlockExam.subject_id == subject_id)
...
def add_modul_exam_db(modul_id: int,student_id: int,chem_score: float,bio_score: float,
    exam_date: datetime,category_correct: List[Any],category_mistake: List[Any]) -> ModulExam:
    ...
    with next(get_db()) as db:
        ...
        exam = ModulExam(modul_id=modul_id,student_id=student_id,chem_score=chem_score,bio_score=bio_score,
            exam_date=exam_date,category_correct=category_correct,category_mistake=category_mistake)
        db.add(exam)
        db.commit()
        db.refresh(exam)
        return exam
...
def update_chem_score_db(student_id: int,modul_id: int,modul_exam_id: int,exam_date: datetime,new_chem_score: float) -> ModulExam:
    """Изменение chem_score по заданным критериям."""
    with next(get_db()) as db:
        exam = (db.query(ModulExam).filter_by(
...
def update_bio_score_db(student_id: int,modul_id: int,
                        modul_exam_id: int,exam_date: datetime,new_bio_score: float) -> ModulExam:
    """Изменение bio_score по заданным критериям."""
    with next(get_db()) as db:
        exam = (db.query(ModulExam)
            .filter_by(modul_exam_id=modul_exam_id,student_id=student_id,
                modul_id=modul_id,exam_date=exam_date)
            .first())
        if not exam:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
...
def get_chem_score_db(student_id: int,modul_id: int,
    modul_exam_id: int,exam_date: datetime) -> float:
    """Получение chem_score по заданным критериям."""
    with next(get_db()) as db:
        exam = (db.query(ModulExam.chem_score)
            .filter_by(modul_exam_id=modul_exam_id,student_id=student_id,
                       modul_id=modul_id,exam_date=exam_date).scalar())
        if exam is None:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                                detail="chem_score not found")
        return exam

def get_bio_score_db(student_id: int,modul_id: int,modul_exam_id: int,exam_date: datetime) -> float:
    """Получение bio_score по заданным критериям."""
    with next(get_db()) as db:
        score = (db.query(ModulExam.bio_score)
            .filter_by(modul_exam_id=modul_exam_id,
                student_id=student_id,modul_id=modul_id,exam_date=exam_date).scalar())
        if score is None:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                                detail="bio_score not found")
        return score

def get_scores_db(student_id: int,modul_id: int,
                  modul_exam_id: int,exam_date: datetime) -> Dict[str, float]:
    """Получение сразу chem_score и bio_score по заданным критериям."""
    with next(get_db()) as db:
        exam = (db.query(ModulExam.chem_score, ModulExam.bio_score)
            .filter_by(modul_exam_id=modul_exam_id,student_id=student_id,modul_id=modul_id,
                exam_date=exam_date).first())
        if not exam:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
...
def get_avg_bio_score_for_student_db(student_id: int) -> float:
    ...
    with next(get_db()) as db:
        avg_bio = (db.query(func.avg(ModulExam.bio_score))
            .filter(ModulExam.student_id == student_id).scalar())
...
def get_all_chem_scores_db(modul_id: int,modul_exam_id: int, exam_date: datetime) -> Dict[int, float]:
    """
    Словарь {student_id: chem_score} для всех студентов
    по заданному modul_id, modul_exam_id и exam_date.
    """
    with next(get_db()) as db:
        rows = (db.query(ModulExam.student_id, ModulExam.chem_score)
            .filter_by(modul_id=modul_id, modul_exam_id=modul_exam_id,exam_date=exam_date).all())
        return {sid: score for sid, score in rows}

def get_all_bio_scores_db(modul_id: int,modul_exam_id: int,
                          exam_date: datetime) -> Dict[int, float]:
    """
    Словарь {student_id: bio_score} для всех студентов
    по заданному modul_id, modul_exam_id и exam_date.
    """
    with next(get_db()) as db:
        rows = (db.query(ModulExam.student_id, ModulExam.bio_score)
            .filter_by(modul_id=modul_id,modul_exam_id=modul_exam_id,exam_date=exam_date).all())
        return {sid: score for sid, score in rows}

def add_topic_test_db(student_id: int,topic_id: int,score: float,attempt_date: datetime) -> TopicTest:
...
def update_topic_test_score_db(student_id: int,topic_id: int,topic_test_id: int,new_score: float) -> TopicTest:
    """Изменение score в TopicTest."""
    with next(get_db()) as db:
        tt = (db.query(TopicTest)
            .filter_by(topic_test_id=topic_test_id,student_id=student_id,topic_id=topic_id).first())
        if not tt:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
...
def get_topic_test_score_db(student_id: int,topic_id: int,topic_test_id: int) -> float:
    ...
    with next(get_db()) as db:
        score = (db.query(TopicTest.score).filter_by(topic_test_id=topic_test_id,
                                                     student_id=student_id,topic_id=topic_id).scalar())
        if score is None:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
...
def get_all_topic_scores_by_topic_db(topic_id: int) -> Dict[int, float]:
    ...
    with next(get_db()) as db:
        rows = (db.query(TopicTest.student_id, TopicTest.score)
            .filter(TopicTest.topic_id == topic_id).all())
...
def get_avg_topic_score_for_student_db(student_id: int,topic_id: int) -> float:
    ...
    with next(get_db()) as db:
        avg = (db.query(func.avg(TopicTest.score)).filter(TopicTest.student_id == student_id,
                TopicTest.topic_id == topic_id).scalar())
        return avg or 0.0

def add_current_rating_db(student_id: int,subject_id: int,topic_id: int,
                          current_score: float,second_current_score: float) -> CurrentRating:
    """
    ...
    with next(get_db()) as db:
        ...
        if not db.query(Topic).get(topic_id):
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f"Topic id={topic_id} not found")
        rating = CurrentRating(student_id=student_id,subject_id=subject_id,
            topic_id=topic_id,current_score=current_score,second_current_score=second_current_score,
...
def update_current_rating_db(rating_id: int,current_score: float | None = None,
    second_current_score: float | None = None) -> CurrentRating:
    """
...
def get_current_score_db(student_id: int,subject_id: int,topic_id: int) -> float:
    ...
    """
    with next(get_db()) as db:
        rating = db.query(CurrentRating).filter_by(student_id=student_id,subject_id=subject_id,topic_id=topic_id).first()
        if not rating:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
...
def get_second_current_score_db(student_id: int,subject_id: int,topic_id: int) -> float:
    ...
    """
    with next(get_db()) as db:
        rating = db.query(CurrentRating).filter_by(student_id=student_id,subject_id=subject_id,
...
def get_avg_sum_scores_by_block_for_student_db(student_id: int,block_id: int) -> float:
    ...
    """
    with (next(get_db()) as db):
        avg_sum = db.query(func.avg(func.coalesce(CurrentRating.current_score, 0) +
                func.coalesce(CurrentRating.second_current_score, 0))).join(Topic, CurrentRating.topic_id == Topic.topic_id
               ).filter(CurrentRating.student_id == student_id,Topic.block_id == block_id).scalar()
...
def get_avg_current_rating_for_block_db(block_id: int) -> dict[int, float]:
    with next(get_db()) as db:
        ...
        rows = (db.query(CurrentRating.student_id,func.avg(CurrentRating.current_score +
                    CurrentRating.second_current_score).label("avg_score"))
            .join(CurrentRating.topic).filter(Topic.block_id == block_id,
                CurrentRating.subject_id == subject_id).group_by(CurrentRating.student_id).all())
